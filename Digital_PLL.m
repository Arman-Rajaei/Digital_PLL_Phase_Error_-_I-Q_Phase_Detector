%%Digital PLL (multiplier phase detector + LPF + NCO/VCO)
clear; close all; clc;
...
Kvco = 5;                % Hz per 1 "control unit" (tuning knob)
Kp = 8;
...
for n = 2:N
    ...
    I_lpf(n) = (1-a)*I + a*I_lpf(n-1);
    Q_lpf(n) = (1-a)*Q + a*Q_lpf(n-1);

    % Phase detector (multiplier)
    phase_err(n) = atan2(Q_lpf(n), I_lpf(n));
...
xlabel('Time (s)'); ylabel('f_{vco}(t) [Hz]'); title('VCO frequency pulled toward input');
...
figure('Name','Compare input vs VCO (zoom)');
idx = 1:500;
